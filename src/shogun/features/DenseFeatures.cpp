...
template <class ST>
const ST*
CDenseFeatures<ST>::get_feature_matrix(int32_t& num_feat, int32_t& num_vec)
{
...
template <class ST>
void CDenseFeatures<ST>::eval_features()
{
...
template <class ST>
Some<CDenseFeatures<ST>>
CDenseFeatures<ST>::view(const SGVector<index_t>& subset)
{
	return wrap(
		static_cast<CDenseFeatures<ST>*>(CFeatures::view(subset).get()));
}

template <class ST>
Some<CDenseFeatures<ST>>
CDenseFeatures<ST>::view(const std::vector<index_t>& subset)
{
	return wrap(
		static_cast<CDenseFeatures<ST>*>(CFeatures::view(subset).get()));
}
...
{
	return wrap(
		static_cast<CDenseFeatures<ST>*>(CFeatures::preprocess(p).get()));
}
